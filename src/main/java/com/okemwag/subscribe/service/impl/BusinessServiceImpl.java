...
public class BusinessServiceImpl implements BusinessService {
  ...
  public List<BusinessDTO> getAllBusinesses(Pageable pageable) {
    ...
    try {
      return businessRepository.findAllActiveWithPagination(pageable).getContent().stream()
  ...
  /**
   * Validates business access for multi-tenant operations This method ensures that operations are
  ...
  public void validateBusinessAccess(Long businessId, Long requestingBusinessId) {
    if (!businessId.equals(requestingBusinessId)) {
      log.warn(
          "Unauthorized access attempt: Business {} tried to access Business {}",
  ...
  public Business getBusinessByEmail(String email) {
    return businessRepository
        .findByEmail(email)
        .orElseThrow(
            () -> new ResourceNotFoundException("Business not found with email: " + email));
  }
