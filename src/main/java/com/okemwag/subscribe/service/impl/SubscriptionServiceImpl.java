...
public class SubscriptionServiceImpl implements SubscriptionService {
  ...
  public SubscriptionDTO createSubscription(CreateSubscriptionDTO dto) {
    log.info(
        "Creating subscription for customer ID: {} with plan ID: {}",
        dto.getCustomerId(),
        dto.getSubscriptionPlanId());
    ...
    SubscriptionPlan subscriptionPlan =
        subscriptionPlanRepository
            .findByIdAndBusinessIdAndActive(
                dto.getSubscriptionPlanId(), customer.getBusiness().getId())
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
    ...
    List<Subscription> existingActiveSubscriptions =
        subscriptionRepository.findByCustomerIdAndStatus(
            dto.getCustomerId(), SubscriptionStatus.ACTIVE);

    boolean hasActivePlanSubscription =
        existingActiveSubscriptions.stream()
    ...
    if (subscriptionPlan.getTrialDays() > 0) {
      ...
      subscription.setEndDate(
          subscription.getStartDate().plusDays(subscriptionPlan.getTrialDays()));
    } else {
    ...
    subscription.setNextBillingDate(
        calculateNextBillingDate(subscription.getStartDate(), subscriptionPlan.getBillingCycle()));

    try {
      Subscription savedSubscription = subscriptionRepository.save(subscription);
      log.info(
          "Successfully created subscription with ID: {} for customer ID: {}",
  ...
  public void cancelSubscription(Long subscriptionId, String cancellationReason) {
    log.info(
        "Cancelling subscription with ID: {} with reason: {}", subscriptionId, cancellationReason);
    ...
    if (subscription.getStatus() == SubscriptionStatus.CANCELLED
      ...
      throw new SubscribeException(
          "Subscription is already " + subscription.getStatus().toString().toLowerCase());
    }
  ...
  public List<SubscriptionDTO> getCustomerSubscriptions(Long customerId) {
    ...
    try {
      ...
      log.error(
          "Error retrieving subscriptions for customer ID {}: {}", customerId, e.getMessage(), e);
  ...
  public SubscriptionDTO renewSubscription(Long subscriptionId) {
    ...
    if (subscription.getStatus() != SubscriptionStatus.ACTIVE
      ...
      throw new SubscribeException(
          "Cannot renew subscription with status: " + subscription.getStatus());
    }
    ...
    try {
      ...
      subscription.setNextBillingDate(
          calculateNextBillingDate(
              currentBillingDate, subscription.getSubscriptionPlan().getBillingCycle()));
  ...
  public List<SubscriptionDTO> getSubscriptionsByBusiness(Long businessId) {
    ...
    try {
      return subscriptionRepository.findByCustomerBusinessId(businessId).stream()
          .map(this::convertToDTO)
          .collect(Collectors.toList());
    } catch (Exception e) {
      log.error(
          "Error retrieving subscriptions for business ID {}: {}", businessId, e.getMessage(), e);
  ...
  public void validateSubscriptionBusinessAccess(Long subscriptionId, Long businessId) {
    ...
    if (!subscription.getCustomer().getBusiness().getId().equals(businessId)) {
      log.warn(
          "Unauthorized access attempt: Business {} tried to access subscription {} from business {}",
          businessId,
          subscriptionId,
          subscription.getCustomer().getBusiness().getId());
  ...
  private LocalDateTime calculateNextBillingDate(
      LocalDateTime currentDate, BillingCycle billingCycle) {
    switch (billingCycle) {
      case MONTHLY:
