...
public class CustomerServiceImpl implements CustomerService {
  ...
  public CustomerDTO registerCustomer(CustomerRegistrationDTO dto) {
    log.info(
        "Registering new customer with email: {} for business ID: {}",
        dto.getEmail(),
        dto.getBusinessId());
    ...
    Business business =
        businessRepository
            .findByIdAndActive(dto.getBusinessId())
            .orElseThrow(
                () ->
    ...
    if (customerRepository.existsByEmailAndBusinessId(dto.getEmail(), dto.getBusinessId())) {
      throw new SubscribeException(
          "Customer with email " + dto.getEmail() + " already exists in this business");
    }
    ...
    try {
      ...
      log.info(
          "Successfully registered customer with ID: {} for business ID: {}",
          savedCustomer.getId(),
          dto.getBusinessId());
  ...
  public CustomerDTO updateCustomer(Long customerId, CustomerUpdateDTO dto) {
    ...
    if (dto.getEmail() != null && !dto.getEmail().equals(customer.getEmail())) {
      if (customerRepository.existsByEmailAndBusinessId(
          dto.getEmail(), customer.getBusiness().getId())) {
        throw new SubscribeException(
            "Customer with email " + dto.getEmail() + " already exists in this business");
      }
      customer.setEmail(dto.getEmail());
  ...
  public List<CustomerDTO> getCustomersByBusiness(Long businessId, Pageable pageable) {
    ...
    try {
      return customerRepository
          .findActiveCustomersByBusinessOrderByCreatedAt(businessId, pageable)
          .getContent()
          .stream()
  ...
  public List<CustomerDTO> searchCustomers(Long businessId, String searchTerm, Pageable pageable) {
    ...
    try {
      return customerRepository
          .searchCustomersByBusiness(businessId, searchTerm, pageable)
          .getContent()
          .stream()
          .map(this::convertToDTO)
          .collect(Collectors.toList());
      ...
      log.error(
          "Error searching customers for business ID {} with term {}: {}",
          businessId,
          searchTerm,
          e.getMessage(),
          e);
  ...
  /**
   * Validates customer access for multi-tenant operations Ensures that operations are performed
   * within the correct business context
   */
  public void validateCustomerBusinessAccess(Long customerId, Long businessId) {
    ...
    if (!customer.getBusiness().getId().equals(businessId)) {
      log.warn(
          "Unauthorized access attempt: Business {} tried to access customer {} from business {}",
          businessId,
          customerId,
  ...
  public CustomerDTO getCustomerByIdAndBusiness(Long customerId, Long businessId) {
    Customer customer =
        customerRepository
            .findByIdAndBusinessIdAndActive(customerId, businessId)
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
  ...
  public List<CustomerDTO> getCustomersWithoutSubscriptions(Long businessId) {
    ...
    return customerRepository.findCustomersWithoutSubscriptionsByBusiness(businessId).stream()
        .map(this::convertToDTO)
        .collect(Collectors.toList());
  }
