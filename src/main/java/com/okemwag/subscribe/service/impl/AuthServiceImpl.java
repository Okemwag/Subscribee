...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponseDTO login(LoginRequestDTO loginRequest) {
        try {
            ...
            BusinessDTO businessDTO = modelMapper.map(
                businessRepository.findById(userPrincipal.getId())
                    .orElseThrow(() -> new ResourceNotFoundException("Business not found")),
                BusinessDTO.class
            );

            // For now, using the same token as refresh token (should be implemented separately)
            String refreshToken = tokenProvider.generateTokenFromBusinessId(
                userPrincipal.getId(), 
                userPrincipal.getEmail(), 
                userPrincipal.getBusinessName()
            );
    ...
    public LoginResponseDTO register(BusinessRegistrationDTO registrationRequest) {
        ...
        // Generate tokens
        String accessToken = tokenProvider.generateTokenFromBusinessId(
            businessDTO.getId(),
    ...
    public LoginResponseDTO refreshToken(String refreshToken) {
        ...
        String newAccessToken = tokenProvider.generateTokenFromBusinessId(
            business.getId(),
            business.getEmail(),
            business.getName()
        );
